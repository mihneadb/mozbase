...
args = options = info = directories = dependencies = parser = versions = dependent_versions = None
...
def check_bumping_dependencies():
    dependent_versions = {}
    missing = []
    types = ['==']
    ...
    # export the data
    globals()['dependent_versions'] = dependent_versions


def bump_desired_packages():
    for name, newversion in versions.items():
        ...
        for index, line in enumerate(lines):
            ...
            parser.error('PACKAGE_VERSION = "%s" not found in %s' % (oldversion,
                                                                     setup_py))
...
method_deps['bump'] = ['pkg_info', 'check_for_master_branch', 'check_for_clean_dir',
                       'parse_pkg_versions', 'check_bumping_dependencies',
                       'check_for_latest_revision', 'bump_desired_packages',
...
method_deps['tag_only'] = ['pkg_info', 'check_for_master_branch',
                           'check_for_clean_dir', 'check_for_latest_revision',
                           'parse_pkg_versions']
method_deps['upload'] = method_deps['tag'] + ['check_for_pypirc', 'upload_to_pypi']


def do_action(action):
    assert action in method_deps
    for dep in method_deps[action]:
        methods[dep]()
...
def main(args=sys.argv[1:]):
    ...
    parser.add_option('--tag', dest='tag',
                      action='store_true', default=False,
                      help="stop after the tagging step, don't upload to pypi")
    parser.add_option('--tag-only', dest='tag_only',
                      action='store_true', default=False,
                      help="just do the tagging step")
